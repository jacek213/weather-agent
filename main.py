...
def ask_gemini_for_endpoint(question, city):
    system_prompt = (
        ...
        "{\n"
        "  \"api_url\": \"<API_URL>\",\n" # embed WEATHER_API_KEY to be replaced in the actual api call. Include the city name in the URL.\n"
...
def ask_gemini_for_answer(question, weather_json, instructions):
    prompt = (
        ...
        "The answer should be in Polish. Should not include technical details, api response, we need to hide that from the end user. Just a concise answer, keep it short, dont say full date and stuff. Respond like a friendly AI assistant but not a robot. "
    )
    contents = types.Content(
        role='user',
...
def main():
    ...
    if question:
        ...
        # Make the API call after swapping the WEATHER_API_KEY
        api_url = api_url.replace("{WEATHER_API_KEY}", WEATHER_API_KEY)
